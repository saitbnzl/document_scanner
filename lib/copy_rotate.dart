...
Image copyRotate(Image src, num angle,
  ...
  if ((nangle % 90.0) == 0.0) {
    ...
    switch (iangle) {
      case 1: // 90 deg.
        final dst =
            Image(src.height, src.width, channels: src.channels, iccp: src.iccProfile);
      ...
      case 2: // 180 deg.
        final dst = Image(src.width, src.height,
            channels: src.channels, iccp: src.iccProfile);
        for (var y = 0; y < dst.height; ++y) {
          for (var x = 0; x < dst.width; ++x) {
  ...
  final dst = Image((ux + vx).toInt(), (uy + vy).toInt(),
      channels: Channels.rgba, iccp: src.iccProfile);
